...
private fun countCells(inp: SimpleGrid<Char>, start: Point, dir: Point): Sint {
	val cells = msop()
	floodFill(start - dir to dir) { (prevPos, dir) ->
		val pos = prevPos + dir
		val value = inp.getBpOrNull(pos)?.v?:return@floodFill emptySet()
		cells += pos
		when (value) {
			'.' -> listOf(pos to dir)
			'/' -> listOf(pos to dir.reflect(PointTransformers.NE))
			'\\' -> listOf(pos to dir.reflect(PointTransformers.NW))
			'-' -> when {
				dir.isHorizontal() -> listOf(pos to dir)
				dir.isVertical() -> listOf(
					pos to Clock.right,
					pos to Clock.left,
				)
				else -> error(dir)
			...
			'|' -> when {
				dir.isHorizontal() -> listOf(
					pos to Clock.up,
					pos to Clock.down,
				)
				dir.isVertical() -> listOf(pos to dir)
				else -> error(dir)
			}
			else -> error("")
		}
