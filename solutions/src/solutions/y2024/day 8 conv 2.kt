...
//	inp = pre(8, 0)
	...
	gs.values.flatMap { v ->
		v.map{it.p}.pairWise().flatMap { (a, b) ->
			listOf(
				a * 2 - b,
				b * 2 - a,
			)
		}
	}.toSet().filter { it in g.bounds }.size log 1
}
...
//	inp = pre(8, 0)
	...
	gs.values.flatMap { v ->
		v.map{it.p}.pairWise().flatMap { (a, b) ->
			listOf(
				a.sequenceInc(a - b).takeWhile { it in g.bounds }.toList(),
				b.sequenceInc(b - a).takeWhile { it in g.bounds }.toList(),
			)
		}.flatten()
	}.toSet().size log 2
}
