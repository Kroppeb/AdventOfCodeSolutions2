...
private fun part1() {
	var inp = getLines(2024, 6).e().grid()

	var cur = inp.find{it.v == '^'}
	var dir = Clock.up
...
private fun part2() {
	...
	val cur = inp.find{it.v == '^'}

	var count = 0;
	for (i in inp.map{it.p}) {
		i log 0
		val check = inp.mutable()
		check[i] = '#'
		val n = check.immutable()
		if (check(n.getBp(cur!!.p))){
			count += 1
		}
	}
	count log 2
