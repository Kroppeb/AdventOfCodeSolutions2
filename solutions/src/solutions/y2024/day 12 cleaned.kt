...
//	var inp = pre(11, 0)
	...
	regions.sumOf {
		...
		it.size.s * it.sumOf { a -> a.p.getQuadNeighbours().count { b -> b !in oo } }
	} log 1
}
...
//	var inp = pre(11, 0)
	...
	regions.sumOf {
		...
		it.size.s * it.sumOf { a -> a.p.getQuadNeighbours().count { b -> (b !in oo && (a.p + (b - a.p).rotateClock() !in oo || a.p + (b - a.p).rotateClock() + (b - a.p) in oo)) } }
	} log 2
}
