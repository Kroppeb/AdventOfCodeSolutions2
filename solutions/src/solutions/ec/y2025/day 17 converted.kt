...
private fun part2() {
	...
	var p = 0
	for (i in 0..50) {
		val r = hob.filter { it.v != '@' && it.p.sqrDistTo(v.p) <= i * i && it.p.sqrDistTo(v.p) > (i - 1) * (i - 1) }
			.map { it.v.toString().sint() }.sum()
		if (r > mx) {
			mx = r
			p = i
		}
...
private fun part3() {
	...
	val rr = dijkstraDist(
		s toH 0.s toH s.sqrDistTo(v).rootCeil(),
		{ (c, x), _ -> c == s && x > 0.s }
	) { (c, x, r), dist ->
		c.getQuadNeighbours().filter { it != v }.map { n ->
			...
			val nr = (n.p - v.p).sqrDist().rootCeil()
			n toH xx toH min(r, nr) to (if (n.v in '0'..'9') n.v.toSint() else 0.s)
		}.filter { it.first.c * 30 > dist && it.first.b >= 0.s }
	} log 0

	val xo = rr!!.path.map { it.a }.toSet()
	hob.print {
		if (it in xo) {
			it.v.color("blue")
		}else if (it.sqrDistTo(v) <= (rr.start.c - 1).pow(2)) {
			it.v.color("red")
		}else {
			it.v.color("bright_black")
		}
	}


	rr.cost * (rr.start.c - 1) log 3
}
